...
function page(string $title,string $body,string $path='',string $description='',array $structured=[],bool $private=false): string {
    ...
    $documentTitle=$title==='Deaf Navi｜手話カフェ'?$title:$title.' | Deaf Navi';
    $robots=$private?'noindex,nofollow':'index,follow';
    $ld='<link rel="stylesheet" href="/directory-community.css?v=20260907tables"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="/directory-safety.js" defer></script><script src="/directory-ui.js?v=20260907cafes2" defer></script>';
    if(!$private) {
...
function cafe_table(array $list): string {
    ...
    foreach($list as $p){
        ...
        $out.='<tr class="dn-cafe-row" data-slug="'.e($p['slug']).'"';foreach(['name','region','type'] as $k)$out.=' data-sort-'.$k.'="'.e(cafe_sort_key($p,$k)).'"';$out.='><th scope="row"><a class="dn-cafe-name" href="'.e(record_path($p)).'">'.e($p['name']).'</a><button type="button" class="dn-expand" data-cafe-expand="'.e($id).'" aria-expanded="false" aria-controls="'.e($id).'" hidden>詳細を開く ＋</button></th><td><span>'.e(($p['country_code']!=='JP'?($p['country_name']?:$p['country_code']).' / ':'').$p['prefecture']).'</span><span class="dn-cell-secondary">'.e($p['city']).'</span></td><td><span class="dn-badge">'.e(SHOP_TYPES[cafe_model($p)['shop_type']]).'</span><span class="dn-cell-secondary dn-operating-state">'.e(cafe_status_label($p)).'</span>'.'</td><td class="dn-hours">'.cafe_schedule_html($p).official_links($p,true).sources_html($p).'</td></tr>';
