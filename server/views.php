...
function page(string $title,string $body,string $path='',string $description='',array $structured=[],bool $private=false,array $meta=[]): string {
    ...
    $ld='<link rel="stylesheet" href="/directory-community.css?v=20260907tables"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="/directory-safety.js?v=20260908loading" defer></script><script src="/directory-ui.js?v=20260908editorial" defer></script>';
    ...
    if(!$private) {
        ...
        $crumbs=[['@type'=>'ListItem','position'=>1,'name'=>'ホーム','item'=>BASE.'/'],['@type'=>'ListItem','position'=>2,'name'=>'手話カフェ','item'=>BASE.'/connect/sign-cafe/']];
        if($path!=='/connect/sign-cafe/') $crumbs[]=['@type'=>'ListItem','position'=>3,'name'=>$title,'item'=>$canonical];
...
function tabs(bool $starbucks=false,bool $overseas=false,bool $decorated=false): string {
    ...
    foreach([['/connect/sign-cafe/','日本の手話カフェ','cup'],['/connect/sign-cafe/overseas/','海外の手話カフェ','globe']] as $i=>[$url,$label,$icon])$out.='<a href="'.$url.'"'.($i===$active?' aria-current="page"':'').'>'.($decorated?'<span class="dn-tab-art">'.cafe_icon($icon).'</span><span>'.$label.'</span>':$label).'</a>';
    return $out.'</nav>';
}
