...
function admin_cafe_scope():string {
    $scope=input($_GET,'scope',20);
    if($scope!=='')return choice($scope,['domestic'=>1,'overseas'=>1]);
...
function admin_shell(string $body,array $user,string $view):string {
    ...
    $items=[['dashboard','','概要'],['records','cafe','手話カフェ','domestic'],['records','cafe','海外の手話カフェ','overseas'],['records','store','スターバックス店舗'],['records','event','スターバックス開催情報'],['submissions','','情報提供・承認待ち'],['audit','','操作履歴']];
    if($user['role']==='admin')$items=[...$items,['access','','アクセスログ'],['users','','ID・権限管理'],['settings','','通知設定'],['preferences','','表示・入力設定']];
    $items[]=['password','','パスワード変更'];
    $nav='';foreach($items as $item){[$v,$k,$label]=$item;$itemScope=$item[3]??'';$active=($view===$v||($view==='edit'&&$v==='records')||($view==='submission'&&$v==='submissions')||($view==='user'&&$v==='users'))&&($k===''||$k===$navKind)&&($itemScope===''||$itemScope===$scope);$nav.='<a href="/admin/?'.e(http_build_query(['view'=>$v]+($k?['kind'=>$k]:[])+($itemScope?['scope'=>$itemScope]:[]))).'"'.($active?' aria-current="page"':'').'>'.e($label).'</a>';}
    return '<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>'.e($title).' | Deaf Navi 管理画面</title><link rel="icon" href="/icons/favicon-48.png?v=20260917b" type="image/png" sizes="48x48"><link rel="stylesheet" href="/directory.css?v=20260908loading"><link rel="stylesheet" href="/admin-dashboard.css?v=20260908b"><link rel="stylesheet" href="/world-cafes.css?v=1"><script src="/access-visit.js?v=20260908b" defer></script><script src="/directory-safety.js?v=20260908loading" defer></script><script src="/directory-ui.js?v=20260905c" defer></script></head><body class="dn-directory dn-dashboard"><a class="dn-skip" href="#main">本文へ</a><div class="admin-layout"><aside class="admin-sidebar"><a class="admin-brand" href="/admin/">Deaf Navi<span>管理ダッシュボード</span></a><nav aria-label="管理メニュー">'.$nav.'</nav><div class="admin-sidebar-foot"><span>'.e($user['username']).' / '.($user['role']==='admin'?'管理者':'編集者').'</span><form action="/admin/" method="post">'.csrf().'<input type="hidden" name="action" value="logout"><button class="secondary">ログアウト</button></form></div></aside><div class="admin-workspace"><header class="admin-topbar"><span>CONTENT MANAGEMENT</span><a href="/connect/sign-cafe/" target="_blank" rel="noopener noreferrer">公開サイトを確認</a></header><main id="main"><div class="admin-page-heading"><p>Deaf Navi Web</p><h1>'.e($title).'</h1></div>'.$body.'</main><footer class="admin-footer">公開・非公開と営業・開催状態は別々に管理します。変更は保存ボタンで確定します。</footer></div></div></body></html>';
}
...
function admin_records_table(string $kind):string {
    $scope=$kind==='cafe'?admin_cafe_scope():'';$scopeQuery=$scope?'&scope='.$scope:'';$country=input($_GET,'country',2);
    $q=input($_GET,'q',200);$pub=input($_GET,'publication',20);$state=input($_GET,'status',30);
    $verification=input($_GET,'verification',20);$freshness=input($_GET,'freshness',12);if($freshness!=='')choice($freshness,['6'=>1,'12'=>1,'unverified'=>1]);
    $states=$kind==='event'?EVENT_STATUSES:STATUSES;
    if($pub!=='')choice($pub,PUBLICATIONS);if($state!=='')choice($state,$states);
    $where=$scope==='overseas'?["kind IN ('cafe','store')"]:($scope==='domestic'?["(kind='cafe' OR (kind='store' AND json_extract(payload,'$.signing_store')=1))"]:['kind=?']);$args=$scope?[]:[$kind];if($scope)$where[]=$scope==='overseas'?"country_code!='JP'":"country_code='JP'";if($country!==''){$where[]='country_code=?';$args[]=$country;}if($q!==''){$where[]="(name LIKE ? ESCAPE '!' OR prefecture LIKE ? ESCAPE '!' OR city LIKE ? ESCAPE '!' OR json_extract(payload,'$.country_name') LIKE ? ESCAPE '!' OR country_code LIKE ? ESCAPE '!')";$like='%'.strtr($q,['!'=>'!!','%'=>'!%','_'=>'!_']).'%';$args=[...$args,$like,$like,$like,$like,$like];}
    foreach(['publication'=>$pub,'status'=>$state] as $k=>$v)if($v!==''){if($k==='status'&&$v==='needs_review'){$where[]="status IN ('needs_review','unknown')";}elseif($k==='status'&&$v==='permanently_closed'){$where[]="status IN ('permanently_closed','closed')";}else{$where[]=$k.'=?';$args[]=$v;}}
    if($freshness!==''){[$condition,$values]=cafe_review_sql($freshness);$where[]=$condition;$args=[...$args,...$values];}
    if($verification!==''){choice($verification,WORLD_VERIFICATIONS);$where[]="COALESCE(json_extract(payload,'$.verification_status'),CASE WHEN json_extract(payload,'$.verification_level')='pending' THEN 'pending' ELSE 'verified' END)=?";$args[]=$verification;}
    ...
    $base=['view'=>'records','kind'=>$kind,'scope'=>$scope,'country'=>$country,'verification'=>$verification,'freshness'=>$freshness,'q'=>$q,'publication'=>$pub,'status'=>$state,'sort'=>$sort,'dir'=>$dir,'limit'=>$size];
    $countries=[];foreach(query("SELECT country_code,payload FROM records WHERE kind IN ('cafe','store') AND country_code!='JP' ORDER BY country_code")->fetchAll() as $r)$countries[$r['country_code']]=(json_decode($r['payload'],true)['country_name']??'')?:$r['country_code'];
    $body='<div class="admin-toolbar"><p>列名で検索結果全体を並べ替えできます。</p><a class="dn-cta" href="/admin/?view=edit&kind='.$kind.$scopeQuery.'">新しく追加</a></div><form method="get" class="admin-filters"><input type="hidden" name="view" value="records"><input type="hidden" name="kind" value="'.$kind.'"><input type="hidden" name="scope" value="'.$scope.'"><input type="hidden" name="sort" value="'.$sort.'"><input type="hidden" name="dir" value="'.$dir.'">'.field('q','店舗名・国・地域で検索',$q).($scope==='overseas'?select_field('country','国・地域',[''=>'すべて']+$countries,$country):'').select_field('verification','情報確認状態',[''=>'すべて']+WORLD_VERIFICATIONS,$verification).select_field('publication','公開状態',[''=>'すべて']+PUBLICATIONS,$pub).select_field('status',$kind==='event'?'開催状態':'営業状態',[''=>'すべて']+$states,$state).select_field('freshness','情報鮮度',[''=>'すべて','6'=>'6か月以上未確認','12'=>'12か月以上未確認','unverified'=>'確認日なし'],$freshness).select_field('limit','表示件数',['25'=>'25件','50'=>'50件','100'=>'100件'],(string)$size).'<button>絞り込む</button><a href="/admin/?view=records&kind='.$kind.$scopeQuery.'">解除</a></form>';
    ...
    foreach($rows as $r){$p=expanded($r);$body.='<tr><th scope="row"><a href="/admin/?view=edit&kind='.e($r['kind']).'&id='.e($r['id']).'">'.e($r['name']).'</a>'.($kind==='event'?'<small>'.e(record($r['store_id'])['name']).'</small>':'').'</th><td>'.e(($r['country_code']!=='JP'?(($p['country_name']??'')?:$r['country_code']).' / ':'').$r['prefecture']).'<small>'.e($r['city']).'</small></td><td><span class="admin-status">'.e(PUBLICATIONS[$r['publication']]).'</span><small>'.e(WORLD_VERIFICATIONS[world_verification($p)]).'</small></td><td>'.e($states[$r['status']]??$r['status']).'</td><td>'.e(($p[$kind==='event'?'event_date':'last_verified_at']??'')?:'未確認').'</td><td>'.e(admin_date($r['updated_at'])).'</td><td><a href="/admin/?view=edit&kind='.e($r['kind']).'&id='.e($r['id']).'">確認・編集</a>'.(publicly_visible($p)?'<a class="admin-secondary-link" href="'.e(record_path($p)).'" target="_blank" rel="noopener noreferrer">公開ページ</a>':'').'</td></tr>';}
...
function admin_submissions_table(bool $preview=false):string {
    ...
    foreach($rows as $r){$p=json_decode($r['payload'],true);$contact=is_cafe_contact($p);$stateLabels=$contact?['pending'=>'確認中','approved'=>'対応済み','rejected'=>'対応不要']:$labels;$body.='<tr><th scope="row">'.e($p['name']??'名称未入力').'</th><td>'.e($contact?'お問い合わせ':(($p['category']??'')==='starbucks'?'スターバックス':'カフェ・その他')).'</td><td>'.e($stateLabels[$r['status']]).'</td><td>'.e(admin_date($r['created_at'])).'</td><td><a href="/admin/?view=submission&id='.e($r['id']).'">内容を確認</a></td></tr>';}
...
function admin_edit_fields(array $p,string $kind):string {
    ...
    $groups=['基本情報'=>['name','name_kana','description','operator'],'営業・開催情報'=>['business_hours','event_schedule','holidays','reservation','sign_support','event_date','start_time','end_time','organizer','partners','conditions','application'],'住所・地域'=>['country_code','country_name','prefecture','city','address','map_url','timezone'],'公式サイト・SNS'=>['official_url','instagram_url','x_url','facebook_url','line_url','youtube_url'],'情報確認・公開の根拠'=>['verification_sources','last_verified_at','published_at'],'詳細設定・管理用情報'=>['subtypes','latitude','longitude','coordinate_accuracy','coordinate_source_url','observation_only','internal_note']];
    $out='';foreach($groups as $title=>$keys){$keys=array_values(array_filter($keys,fn($k)=>isset($all[$k])));if(!$keys)continue;$out.='<fieldset class="admin-fieldset"><legend>'.e($title).'</legend>';
        if($title==='詳細設定・管理用情報')$out.='<p class="dn-muted">座標・内部メモなど。これらの管理用項目は一般向けの店舗本文には表示しません。</p>';
        if($title==='住所・地域'&&$kind!=='event')$out.='<p class="dn-muted">掲載先は国コードで決まります。日本はJP、海外はUS・GBなど。国・地域名と都市を入力し、州がない地域は州を空欄にできます。営業時間は現地時間、手話対応には現地の手話や筆談の内容を記入してください。</p>';
        ...
        foreach($keys as $k){
            ...
            $out.=field($k,$all[$k],$p[$k]??'',$type,$k==='name'||($kind!=='event'&&in_array($k,['country_code','country_name','city'],true)&&($p['country_code']??'')!=='JP'));
        }$out.='</div></fieldset>';
    }return $out;
}
