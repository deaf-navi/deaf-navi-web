...
function admin_shell(string $body,array $user,string $view):string {
    ...
    $scope=admin_cafe_scope();$navKind=$kind;if($kind==='store'&&$scope==='overseas'&&input($_GET,'id',64)!==''&&!empty(expanded(record(input($_GET,'id',64)))['signing_store']))$navKind='cafe';if($navKind==='cafe'&&$scope==='overseas'&&in_array($view,['records','edit'],true))$title=$view==='records'?'海外の手話カフェ一覧':'海外の手話カフェの編集';
...
function admin_records_table(string $kind):string {
    ...
    $where=$scope==='overseas'?["(kind='cafe' OR (kind='store' AND json_extract(payload,'$.signing_store')=1))"]:['kind=?'];$args=$scope==='overseas'?[]:[$kind];if($scope)$where[]=$scope==='overseas'?"country_code!='JP'":"country_code='JP'";if($country!==''){$where[]='country_code=?';$args[]=$country;}if($q!==''){$where[]="(name LIKE ? ESCAPE '!' OR prefecture LIKE ? ESCAPE '!' OR city LIKE ? ESCAPE '!' OR json_extract(payload,'$.country_name') LIKE ? ESCAPE '!' OR country_code LIKE ? ESCAPE '!')";$like='%'.strtr($q,['!'=>'!!','%'=>'!%','_'=>'!_']).'%';$args=[...$args,$like,$like,$like,$like,$like];}
    ...
    $base=['view'=>'records','kind'=>$kind,'scope'=>$scope,'country'=>$country,'q'=>$q,'publication'=>$pub,'status'=>$state,'sort'=>$sort,'dir'=>$dir,'limit'=>$size];
    $countries=[];foreach(query("SELECT country_code,payload FROM records WHERE (kind='cafe' OR (kind='store' AND json_extract(payload,'$.signing_store')=1)) AND country_code!='JP' ORDER BY country_code")->fetchAll() as $r)$countries[$r['country_code']]=(json_decode($r['payload'],true)['country_name']??'')?:$r['country_code'];
    $body='<div class="admin-toolbar"><p>列名で検索結果全体を並べ替えできます。</p><a class="dn-cta" href="/admin/?view=edit&kind='.$kind.$scopeQuery.'">新しく追加</a></div><form method="get" class="admin-filters"><input type="hidden" name="view" value="records"><input type="hidden" name="kind" value="'.$kind.'"><input type="hidden" name="scope" value="'.$scope.'"><input type="hidden" name="sort" value="'.$sort.'"><input type="hidden" name="dir" value="'.$dir.'">'.field('q','店舗名・国・地域で検索',$q).($scope==='overseas'?select_field('country','国・地域',[''=>'すべて']+$countries,$country):'').select_field('publication','公開状態',[''=>'すべて']+PUBLICATIONS,$pub).select_field('status',$kind==='event'?'開催状態':'営業状態',[''=>'すべて']+$states,$state).select_field('limit','表示件数',['25'=>'25件','50'=>'50件','100'=>'100件'],(string)$size).'<button>絞り込む</button><a href="/admin/?view=records&kind='.$kind.$scopeQuery.'">解除</a></form>';
    ...
    foreach($rows as $r){$p=expanded($r);$body.='<tr><th scope="row"><a href="/admin/?view=edit&kind='.e($r['kind']).'&id='.e($r['id']).'">'.e($r['name']).'</a>'.($kind==='event'?'<small>'.e(record($r['store_id'])['name']).'</small>':'').'</th><td>'.e(($r['country_code']!=='JP'?(($p['country_name']??'')?:$r['country_code']).' / ':'').$r['prefecture']).'<small>'.e($r['city']).'</small></td><td><span class="admin-status">'.e(PUBLICATIONS[$r['publication']]).'</span></td><td>'.e($states[$r['status']]??$r['status']).'</td><td>'.e(($p[$kind==='event'?'event_date':'last_verified_at']??'')?:'未確認').'</td><td>'.e(admin_date($r['updated_at'])).'</td><td><a href="/admin/?view=edit&kind='.e($r['kind']).'&id='.e($r['id']).'">確認・編集</a>'.(publicly_visible($p)?'<a class="admin-secondary-link" href="'.e(record_path($p)).'" target="_blank" rel="noopener noreferrer">公開ページ</a>':'').'</td></tr>';}
...
function admin_overview():string {
    $metrics=[['確認待ちの投稿',"SELECT count(*) FROM submissions WHERE status='pending'",'/admin/?view=submissions&status=pending'],['公開中の手話カフェ',"SELECT count(*) FROM records WHERE kind='cafe' AND country_code='JP' AND publication='public'",'/admin/?view=records&kind=cafe&scope=domestic&publication=public'],['公開中の海外の手話カフェ',"SELECT count(*) FROM records WHERE (kind='cafe' OR (kind='store' AND json_extract(payload,'$.signing_store')=1)) AND country_code!='JP' AND publication='public'",'/admin/?view=records&kind=cafe&scope=overseas&publication=public'],['スターバックス店舗',"SELECT count(*) FROM records WHERE kind='store' AND publication!='deleted'",'/admin/?view=records&kind=store'],['開催情報',"SELECT count(*) FROM records WHERE kind='event' AND publication!='deleted'",'/admin/?view=records&kind=event']];
