...
function domestic_cafe_page():string {
    ...
    $body='<div class="dn-cafe-navigation">'.tabs(false,false,false).'<nav class="dn-section-links" aria-label="手話カフェの探し方">'.cafe_guide_link('/connect/sign-cafe/map/','地図から探す','map').cafe_guide_link('#cafe-regions','地域から探す','pin').cafe_guide_link('#cafe-guide','初めての方へ','book').cafe_guide_link('#cafe-policy','掲載・更新方針','edit').'</nav></div><div class="dn-directory-layout">'.cafe_filter_panel(domestic_filters($f)).'<section id="cafes" class="dn-directory-results" aria-labelledby="cafe-list-heading"><h2 id="cafe-list-heading" class="dn-visually-hidden">手話カフェを探す</h2>'.'<div class="dn-result-bar"><p class="dn-result" role="status">該当 <strong>'.$total.'</strong>件</p><a href="/connect/sign-cafe/map/">'.cafe_icon('map').'地図から探す</a></div><p class="dn-muted dn-results-note">「営業中」は掲載情報の確認状態です。今この時刻に開いていることを示しません。訪問前に公式サイト・SNSをご確認ください。</p>';
    ...
    $body.='</nav></section></div>'.cafe_notice_link().cafe_discovery_guide($all).cafe_listing_notice();
