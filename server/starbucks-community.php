...
function starbucks_form(string $storeId=''):string {
    ...
    $stores=[''=>'掲載にない店舗・店舗名を入力'];foreach(visible_records() as $p)if($p['kind']==='store'&&$p['country_code']==='JP')$stores[$p['id']]=$p['name'];
...
function starbucks_submission(array $post):array {
    ...
    if($id!==''){$r=query("SELECT * FROM records WHERE id=? AND kind='store' AND country_code='JP' AND publication='public'",[$id])->fetch();if(!$r||!publicly_visible($store=expanded($r)))fail('掲載中の国内店舗を選んでください。');}
