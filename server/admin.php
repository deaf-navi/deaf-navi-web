...
function admin_page(): string {
    ...
    elseif($view==='password') {
        ...
        $body.=field('slug','個別URL名（公開後は原則変更しない）',$p['slug']??'','text',true).select_field('publication','公開状態',PUBLICATIONS,$p['publication']??'pending').select_field('status',$kind==='event'?'開催状態':'営業状態',$kind==='event'?EVENT_STATUSES:STATUSES,$p['status']??($kind==='event'?'date_unknown':'unknown')).select_field('verification_level','確認に使った情報源の種別',LEVELS,$p['verification_level']??'pending');
        ...
        if($kind!=='event')$body.=world_admin_fields($p);
    ...
    return admin_shell($body,$user,$view);
}
...
function admin_action(): void {
    ...
    if($action==='update_user') {admin_update_user($u);}
    elseif($action==='admin_preferences') {admin_save_preferences();}
    elseif($action==='create_user') {
        ...
        if(isset($_POST['confirm_password'])&&$pw!==input($_POST,'confirm_password',256))fail('初期パスワードの確認入力が一致しません。');
        ...
        require_user(true);fail('ID一覧を再読み込みし、権限・利用状態の画面で確認して保存してください。',409);
